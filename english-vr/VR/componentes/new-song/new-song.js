...
document.addEventListener('DOMContentLoaded', function () {
    ...
    AFRAME.registerComponent('new-song', {
        schema: {
        ...
        init: function () {
            ...
            // Botón para previsualizar el YouTube URL (Opción B del Requerimiento 003, sección 4)
            ...
            // Advertencia de derechos de autor (Requerimiento 003, sección 4.4)
            const warning = document.createElement('a-text');
            warning.setAttribute('value', 'Verifica que tienes derecho a usar este contenido antes de copiarlo a videos/karaoke/. Ver Requerimiento 003, seccion 4.4.');
            ...
            // Raycast manual de mouse (clic exacto bajo el puntero), igual que karaoke-vr/evaluacion-vr:
        ...
        _saveSong: function () {
            ...
            this._statusText.setAttribute('color', '#ffffcc');
            this._statusText.setAttribute('value', 'Guardando...');

            const body = new URLSearchParams({ titulo: titulo, autor: autor, archivo: archivo });

            fetch('/A-frame/Proyecto/backend/modelos/canciones/registrar_canciones.php', {
                method: 'POST',
                ...
                .then(({ ok, json }) => {
                    if (!ok || (json && json.error)) {
                        throw new Error((json && json.error) || 'Error desconocido al guardar.');
                    }
                    ...
                    try { window.dispatchEvent(new CustomEvent('cancion-agregada', { detail: json })); } catch (e) { /* ignore */ }
                    ...
                    fetch('./videos/karaoke/' + encodeURIComponent(archivo), { method: 'HEAD' })
                        .then((res) => {
                            if (!res.ok) {
                                this._statusText.setAttribute('value', this._statusText.getAttribute('value') + ' Aviso: no se encontro el archivo en videos/karaoke/.');
                            }
                        })
                ...
                .catch((err) => {
                    this._statusText.setAttribute('color', '#ff8888');
                    this._statusText.setAttribute('value', err.message || 'Error al guardar la cancion.');
                });
        }
    });
