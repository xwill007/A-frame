...
AFRAME.registerComponent('formulario-registro-vr', {
    ...
    handleFieldClick: function(field) {
        ...
        switch(fieldId) {
            case 'name-field':
                sampleText = 'Usuario VR';
                break;
            case 'email-field':
                sampleText = 'usuario@vr.com';
                break;
            case 'password-field':
                sampleText = '••••••••••';
                break;
    ...
    selectLevel: function(selectedBtn) {
        ...
        levelBtns.forEach(btn => {
            ...
            btn.setAttribute('animation__deselect', 'property: scale; to: 1 1 1; dur: 200');
        });
        ...
        const level = selectedBtn.id.replace('-btn', '');
    ...
    handleRegister: function() {
        ...
        const nameField = this.el.querySelector('#name-field a-text:last-child');
        ...
        if (registerBtn) {
            registerBtn.setAttribute('animation__processing', 'property: rotation; to: 0 360 0; dur: 2000; loop: 3');
        }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Sistema de formulario VR cargado');
    
    // Controles de teclado para el formulario VR
        ...
        if (registroComponent) {
            ...
            if (formComponent) {
                switch(e.key) {
                    ...
                    case '1':
                        if (formComponent.isFormVisible) {
                            const beginnerBtn = registroComponent.querySelector('#beginner-btn');
                            if (beginnerBtn) formComponent.selectLevel(beginnerBtn);
                        }
                        break;
                    case '2':
                        if (formComponent.isFormVisible) {
                            const intermediateBtn = registroComponent.querySelector('#intermediate-btn');
                            if (intermediateBtn) formComponent.selectLevel(intermediateBtn);
                        }
                        break;
                    case '3':
                        if (formComponent.isFormVisible) {
                            const advancedBtn = registroComponent.querySelector('#advanced-btn');
                            if (advancedBtn) formComponent.selectLevel(advancedBtn);
                        }
                        break;
                }
            }
    ...
    console.log('Controles del formulario VR disponibles:');
    console.log('- Click: Interactuar con campos y botones');
    console.log('- 1, 2, 3: Seleccionar nivel (cuando formulario esté abierto)');
    console.log('- Enter: Registrarse (cuando formulario esté abierto)');
